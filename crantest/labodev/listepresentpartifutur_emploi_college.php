...
echo "college"."\t"."HE"."\t"."codeindividu"."\t"."numsejour"."\t"."codelabintel"."\t"."Etat"."\t"."Suivi par"."\t"."debut"."\t"."fin"."\t"."Nom"."\t"."Prenom"."\t"."Nom JF"."\t".
			"Date naiss"."\t"."Ville naiss"."\t"."Pays naiss"."\t"."Nationalité"."\t".
			...
			"\t"."Co-tutelle"."\t"."Pays co-tutelle"."\t"."Master, Etab., année"."\t"."Dipl. ant. Master"."\t"."Etat"."\t"."ED these"."\t"."Annee"."\t"."Date inscr.".
...
{ $row_rs_individu["libpays_naiss"]=$tab_pays[$row_rs_individu["codepays_naiss"]]["libpays"];
  ...
  { $listetheme.=$libtheme.", ";
	...
	echo $row_rs_individu["codecollege"]."\t".($row_rs_individu["hors_effectif"]=='oui'?'oui':'')."\t".$codeindividu."\t".$numsejour."\t".txt2type($row_rs_individu["codelabintel"],$type_fichier)."\t".$row_rs_individu["partipresentfutur"]."\t".
				...
				($row_rs_individu['associe']=='oui'?"Associé":"")."\t".$row_rs_individu['esthdr']."\t".$row_rs_individu['libed']."\t".txt2type(aaaammjj2jjmmaaaa($row_rs_individu["date_hdr"],'-'),$type_fichier)."\t".
  ...
  { $row_rs_individuemploi=$tab_individuemploi[$codeindividu][$numsejour];
	...
	if($a_un_emploi)
	{ echo txt2type($row_rs_individuemploi["datedeb_emploi"],$type_fichier)."\t".txt2type($row_rs_individuemploi["datefin_emploi"],$type_fichier)."\t".txt2type($row_rs_individuemploi["libetab"],$type_fichier)."\t".
	...
	{ $row_rs_individusujet=$tab_individusujet[$codeindividu][$numsejour];
		echo 	"\t".txt2type($row_rs_individusujet["titresujet"],$type_fichier);
		...
		for($i=1;$i<=3;$i++)
		{ if(isset($tab_dir[''.$i]))
			{ echo "\t".txt2type($tab_dir[''.$i]['prenom']." ".$tab_dir[''.$i]['nom'],$type_fichier)."\t".$tab_dir[''.$i]['taux_encadrement'];
			}
			else
			{ echo "\t"."\t";
			}
		}
		echo 	"\t".txt2type($row_rs_individusujet['autredir1'],$type_fichier)."\t".txt2type($row_rs_individusujet['autredir2'],$type_fichier);
		...
		{ $row_rs_individuthese=$tab_individuthese[$codeindividu][$numsejour];
			...
			echo "\t".(strtolower($row_rs_individuthese["cotutelle"])=='oui'?txt2type($row_rs_individuthese["cotutelle"],$type_fichier)."\t".txt2type($row_rs_individuthese["libpays_cotutelle"],$type_fichier):"\t"); 
			echo "\t".txt2type($row_rs_individusujet["libmaster_obtenu"],$type_fichier).
					 "\t".txt2type($row_rs_individusujet["diplome_dernier_lib"],$type_fichier);
			echo "\t".$etat_these."\t".txt2type($row_rs_individuthese["libed"],$type_fichier)."\t".$annee_these;
			echo "\t".txt2type(aaaammjj2jjmmaaaa($row_rs_individuthese["date_preminscr"],"/"),$type_fichier);
			echo "\t".txt2type(aaaammjj2jjmmaaaa($row_rs_individuthese["date_suivi_comite_selection_12_mois"],"/"),$type_fichier);
			echo "\t".txt2type(aaaammjj2jjmmaaaa($row_rs_individuthese["date_suivi_comite_selection_30_mois"],"/"),$type_fichier);
			echo "\t".txt2type(aaaammjj2jjmmaaaa($row_rs_individuthese["date_soutenance"],"/"),$type_fichier)."\t".txt2type($row_rs_individuthese["duree_mois_these"],$type_fichier)."\t".txt2type($row_rs_individuthese["etab_these"],$type_fichier);
			echo "\t".txt2type($row_rs_individuthese["titre_these"],$type_fichier)."\t".txt2type($row_rs_individuthese["jury_rapp1_these"],$type_fichier)."\t".txt2type($row_rs_individuthese["jury_rapp2_these"],$type_fichier).
					 "\t".txt2type($row_rs_individuthese["jury_autres_membres_these"],$type_fichier);
			echo "\t".txt2type($row_rs_individuthese["profession_postdoc"],$type_fichier)."\t".txt2type($row_rs_individuthese["employeur_postdoc"],$type_fichier);
			echo "\t".txt2type($row_rs_individuthese["ville_postdoc"],$type_fichier)."\t".txt2type($row_rs_individuthese["libpays_postdoc"],$type_fichier).
