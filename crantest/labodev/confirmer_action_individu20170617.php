...
{ //si un seul sejour => suppression de l'individu sinon le sejour concerne
  ...
  $row_rs=mysql_fetch_assoc($rs);
	...
	// droits sur des programmes particuliers : non supprimable
	$rs=mysql_query("select codeindividu from individuprog where codeindividu=".GetSQLValueString($codeindividu, "text")) or die(mysql_error());
	if($row_rs=mysql_fetch_assoc($rs) && $nbsejour<=1)
