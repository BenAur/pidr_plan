  ...
  */
  ...
  { foreach($tab_acteurs_par_role as $ieme_acteur=>$tab_info_un_acteur_du_role)
	...
	{ $query_rs_individuaction= "select * from individuaction".
		...
		{ if($GLOBALS['mode_exploit']=='test' || $GLOBALS['mode_exploit']=='demo')
			{ $to.=", TEST DIRECTION <".$GLOBALS['emailTest'].">, TEST AP <".$GLOBALS['emailTest'].">, TEST Serv. Info. <".$GLOBALS['emailTest'].">";//test
			}
			else
			{ $to.=(est_mail($GLOBALS['emailDIRECTION'])?", ".$GLOBALS['emailDIRECTION']:"").(est_mail($GLOBALS['emailACMO'])?", ".$GLOBALS['emailACMO']:"").(est_mail($GLOBALS['emailSID'])?", ".$GLOBALS['emailSID']:"");
			}
		}
		// 20170328
	}
...
{ // mail de dde de declaration fsd
  ...
  if(isset($rs))mysql_free_result($rs);
	...
	$message.="<br><br>Restant &agrave; votre disposition pour tout compl&eacute;ment d'information, veuillez agr&eacute;er mes salutations distingu&eacute;es.";
	...
	// le développeur
	if(!array_key_exists(strtolower($GLOBALS['webMaster']['email']),$tab_mail_unique) && est_mail($GLOBALS['webMaster']['email']))// 20170328
	{ $to.=($to==""?"":", ").$GLOBALS['webMaster']['nom'].' <'.$GLOBALS['webMaster']['email'].'>';
	}
...
{ $sujetaffecte=false;
	...
	{ $sujetaffecte=true;
		...
		$query_rs="select listediffusion.nomliste as email from listediffusion,structure where structure.codestructure=listediffusion.codestructure".
								" and structure.codelib=".GetSQLValueString('srh', "text");
		$rs=mysql_query($query_rs) or die(mysql_error());
		if($row_rs=mysql_fetch_assoc($rs))
		{ $tab_destinataires['srh']=array('prenomnom'=>'cran-rh','email'=>$row_rs['email']);
		}
	}
	...
	$from = $GLOBALS['Serveur12+']['nom'].' <'.$GLOBALS['Serveur12+']['email'].'>';
	$replyto= $tab_infouser['email']!=''?($tab_infouser['prenom'].' '.$tab_infouser['nom'].' <'.$tab_infouser['email'].'>'):($GLOBALS['Serveur12+']['nom'].' <'.$GLOBALS['Serveur12+']['email'].'>');
	
	$to="";
	$first=true;
	foreach($tab_destinataires as $un_destinataire=>$tab_un_destinataire)
	{ if(!array_key_exists(strtolower($tab_un_destinataire['email']),$tab_mail_unique) && est_mail($tab_un_destinataire['email']))// 20170328
		{ $tab_mail_unique[strtolower($tab_un_destinataire['email'])]=$tab_un_destinataire['email'];
			$to.=($first?"":", ").$tab_un_destinataire['prenomnom'].' <'.$tab_un_destinataire['email'].'>';
			$first=false;
		}
...
{ $message="";
	$query_rs="select structure.libcourt_fr as libtheme from projet,structure".
						" where projet.codetheme=structure.codestructure".
						" and projet.codeprojet=".GetSQLValueString($codeprojet, "text");
	$rs=mysql_query($query_rs) or die(mysql_error());
	$row_rs=mysql_fetch_assoc($rs);
	$libtheme=$row_rs['libtheme']; 	

	if($typevalidation=='brouillon')
	{ $message= addslashes("Le sujet ne sera plus visible par les autres membres");
	}
	else
	{ $message= "Un message de publication de ce projet va &ecirc;tre adress&eacute; &agrave; la liste de diffusion ".$libtheme;
	}
  if(isset($rs))mysql_free_result($rs);
	return $message;
}
...
{ $tab_destinataires=array();
	...
	$message="";
 	...
 	$rs_projet=mysql_query("select projet.*, cont_classif.libcourtclassif as libclassif, proj_typeprojet.codelibtypeprojet, proj_typeprojet.libtypeprojet".
												 " from projet, cont_classif, proj_typeprojet".
												 " where projet.codeclassif=cont_classif.codeclassif and projet.codetypeprojet=proj_typeprojet.codetypeprojet".
												 " and projet.codeprojet=".GetSQLValueString($codeprojet,"text")) or die(mysql_error());
	$row_rs_projet=mysql_fetch_assoc($rs_projet);
	
	$query_rs="select structure.libcourt_fr as libtheme, listediffusion.nomliste from projet,structure,listediffusion".
						" where projet.codetheme=structure.codestructure and structure.codestructure=listediffusion.codestructure".
						" and projet.codeprojet=".GetSQLValueString($codeprojet, "text");
	$rs=mysql_query($query_rs) or die(mysql_error());
	$row_rs=mysql_fetch_assoc($rs);
	$libtheme=$row_rs['libtheme'];
	$nomliste= $row_rs['nomliste'];
	if($row_rs_projet['demandeaccompagnement']=='oui')
	{ $roles='admingestfin,gestprojet';
	}
	else
	{ $roles='du,admingestfin,gestprojet';
	}
	$rs_acteur=mysql_query( "select individu.codeindividu,nom,prenom,email from individu,structureindividu,structure".
													" where individu.codeindividu=structureindividu.codeindividu".
													" and structureindividu.codestructure=structure.codestructure".
													" and FIND_IN_SET(codelib,".GetSQLValueString($roles, "text").")") or die(mysql_error());
												
	while($row_rs_acteur=mysql_fetch_assoc($rs_acteur))
	{ $tab_destinataires[$row_rs_acteur['codeindividu']]=array('prenomnom'=>$row_rs_acteur['prenom'].' '.$row_rs_acteur['nom'],'email'=>$row_rs_acteur['email']);
	}

	$subject="Nouveau projet ".$row_rs_projet['titrecourt']." d&eacute;pos&eacute; par ".$tab_infouser['prenom']." ".$tab_infouser['nom'];
	// contenu du message de validation
	$message="Bonjour,<br><br>";
	...
	$message.="<br><b>Intitul&eacute; court :&nbsp;</b>".$row_rs_projet['titrecourt'];
	$message.="<br><b>Intitul&eacute; : </b>".$row_rs_projet['titre'];
	$message.="<br><b>Contexte de d&eacute;p&ocirc;t :&nbsp;</b>".$row_rs_projet['libclassif'];
	$message.="<br><b>Type :&nbsp;</b>".$row_rs_projet['libtypeprojet'].'&nbsp;'.$row_rs_projet['detailtypeprojet'];
	if($row_rs_projet['codelibtypeprojet']=='contratrecherche')
	{ $message.='<br><b>Date de d&eacute;but pr&eacute;vue :&nbsp;</b>'.aaaammjj2jjmmaaaa($row_rs_projet['datedeb_projet'],"/").'&nbsp;<b>Dur&eacute;e en mois :&nbsp;</b>'.$row_rs_projet['duree_mois'];
	}
	else if($row_rs_projet['codelibtypeprojet']=='appelprojet')
	{ $message.='<br><b>Date limite de d&eacute;p&ocirc;t du projet :&nbsp;</b>'.aaaammjj2jjmmaaaa($row_rs_projet['datelimite_depot_projet'],"/");
	}
	if($row_rs_projet['datedepot_projet']!='')
	{ $message.='<br><b>Date de d&eacute;p&ocirc;t du projet si d&eacute;j&agrave; d&eacute;pos&eacute; :&nbsp;</b>'.aaaammjj2jjmmaaaa($row_rs_projet['datedepot_projet'],"/");
	}
	$message.="<br><b>Montant total :&nbsp;</b>".$row_rs_projet['montant_total']."&nbsp;<b>Montant laboratoire :&nbsp;</b>".$row_rs_projet['montant_labo'];
	$message.="<br><br><b>Description :&nbsp;</b>".$row_rs_projet['descr'];
	$message.="<br><br><b>Sp&eacute;cificit&eacute;s :&nbsp;</b>".$row_rs_projet['specificites'];
	$message.="<br><br><b>Demande d&rsquo;accompagnement de montage financier/juridique &nbsp; :&nbsp;</b>".strtoupper($row_rs_projet['demandeaccompagnement']);
	...
	$replyto= $tab_infouser['prenom']." ".$tab_infouser['nom'].' <'.$tab_infouser['email'].'>';
	$to="";
	// la liste de diffusion du theme
	$to=$nomliste;
	foreach($tab_destinataires as $un_destinataire=>$tab_un_destinataire)
	{ if(!array_key_exists(strtolower($tab_un_destinataire['email']),$tab_mail_unique) && est_mail($tab_un_destinataire['email']))// 20170328
		{ $tab_mail_unique[strtolower($tab_un_destinataire['email'])]=$tab_un_destinataire['email'];
			$to.=", ".$tab_un_destinataire['prenomnom'].' <'.$tab_un_destinataire['email'].'>';
		}
	}	
	//le user expediteur s'il n'est pas deja dans $to
	if(!array_key_exists(strtolower($tab_infouser['email']),$tab_mail_unique) && est_mail($tab_infouser['email']))
...
function mail_accompagnement_projet($codeprojet,$codeuser,$info_accompagnement_projet)
{ 
	$tab_infouser=get_info_user($codeuser);
	$tab_destinataires=array();
	$tab_mail_unique=array();//evite d'avoir deux fois le meme destinataire (meme adresse mail) dans le champ To:
	...
	$rs_projet=mysql_query($query_rs_projet);
	$row_rs_projet=mysql_fetch_assoc($rs_projet);
	$tab_destinataires[$row_rs_projet['emailreferent']]=array('prenomnom'=>$row_rs_projet['prenomreferent'].' '.$row_rs_projet['nomreferent'],'email'=>$row_rs_projet['emailreferent']);	
	
	$subject="Accompagnement de montage de projet ".$row_rs_projet['titrecourt'].' ('.$row_rs_projet['prenomreferent'].' '.$row_rs_projet['nomreferent'].')';
	$message=nl2br($info_accompagnement_projet);
	$subject=	html_entity_decode($subject);
	...
	<head>
	<meta http-equiv="content-type" content="text/html; charset=iso-8859-1">
	<title>Accompagnement projet</title>
	<link rel="stylesheet" href="'.$GLOBALS['racine_site_web_labo'].'styles/normal.css">
	</head>
	<body>'.
	$message.'
...
// mail envoye aux acteurs concernes du projet 
function mail_traite_accompagnement_projet($codeprojet,$codeuser)
{ 
	$tab_infouser=get_info_user($codeuser);
	$tab_destinataires=array();
	$tab_mail_unique=array();
	...
	$subject="Accompagnement de montage de projet trait&eacute; ".$row_rs_projet['titrecourt'].' ('.$row_rs_projet['prenomreferent'].' '.$row_rs_projet['nomreferent'].')';
	// contenu du message de validation
	$message.="Bonjour,<br><br>";
	$message.="L'accompagnement du projet suivant a &eacute;t&eacute; trait&eacute; par ".$tab_infouser['prenom']." ".$tab_infouser['nom']." :";
	...
	{ if(!array_key_exists(strtolower($tab_un_destinataire['email']),$tab_mail_unique) && est_mail($tab_un_destinataire['email']))// 20170328
		{ $tab_mail_unique[strtolower($tab_un_destinataire['email'])]=$tab_un_destinataire['email'];
			$to.=($to==""?"":", ").$tab_un_destinataire['prenomnom'].' <'.$tab_un_destinataire['email'].'>';
		}
	}	
	//le user expediteur s'il n'est pas deja dans $to
	if(!array_key_exists(strtolower($tab_infouser['email']),$tab_mail_unique) && est_mail($tab_infouser['email']))
